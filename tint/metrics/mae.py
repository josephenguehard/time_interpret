...
def _mae(
    logits_original: Tensor, logits_pert: Tensor, target: Tensor
) -> Tensor:
    return (
        (
            _select_targets(logits_original, target).reshape(
                len(logits_original), -1
            )
            - _select_targets(logits_pert, target).reshape(
                len(logits_pert), -1
            )
        )
        .abs()
...
def mae(
    ...
    This metric measures the mean absolute error between the outputs of
    the model using the original inputs and perturbed inputs by removing
    or only keeping the topk most important features. Lower is better.
    ...
    Returns:
        (float): The mean absoule error metric.
